...
def main(demo_type, n_clusters, n_clouds=None):
			...
			if demo_name != "ar_demo":
				for seg_name in demofile[demo_name]:
					if seg_name != 'done':
						...
						print demo_name, seg_name
						seg_num += 1
						if n_clouds is not None and seg_num >= n_clouds: break 

	except IOError:
		keys, clouds = extract_clouds(demofile)
		sm = similarity_matrix(clouds,n_clouds)
...
if __name__ == "__main__":
	...
	parser.add_argument("--num_clouds", type=int, default=-1)
	args = parser.parse_args()

	if args.num_clouds < 0:
		nc = None
	else:
		nc = args.num_clouds
	main(args.demo_type, args.num_clusters, nc)

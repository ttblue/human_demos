...
from hd_rapprentice.registration import loglinspace, ThinPlateSpline, fit_ThinPlateSpline
import tps
...
def fit_ThinPlateSpline_corr(x_nd, y_md, corr_nm, l, rot_reg, x_weights=None):
    ...
    f = fit_ThinPlateSpline(x_nd, xtarg_nd, bend_coef = l, rot_coef = rot_reg, wt_n = wt_n)
    #f = fit_ThinPlateSpline(x_nd, xtarg_nd, bend_coef = 0.1, rot_coef = 1e-3, wt_n = wt_n)
    f._bend_coef = l
    f._wt_n = wt_n
    f._rot_coef = rot_reg
...
def calc_segment_corr(rope_nodes1, pts_segmentation_inds0, pts_segmentation_inds1):
    n = int(pts_segmentation_inds0[-1])
    m = int(pts_segmentation_inds1[-1])
    ...
    for i, (i_start0, i_end0, i_start1, i_end1) in enumerate(zip(pts_segmentation_inds0[:-1], pts_segmentation_inds0[1:], pts_segmentation_inds1[:-1], pts_segmentation_inds1[1:])):
        ...
        if i_end1-i_start1 > 1:
            heights = np.apply_along_axis(np.linalg.norm, 1, np.diff(rope_nodes1[int(i_start1):int(i_end1),:], axis=0))
            lengths = np.r_[lengths, heights]
        summed_lengths = np.cumsum(lengths)
        corr_nm[int(i_start0):int(i_end0),int(i_start1):int(i_end1)] = math_utils.interp_mat(np.linspace(0, summed_lengths[-1], int(i_end0-i_start0)), summed_lengths)
    return corr_nm
...
def tile(A, tile_pattern):
    ...
    return B
                            ...
                            #demo info                     #seg info                     #demo_cloud     #sim_cloud
def tps_segment_registration(rope_nodes_or_crossing_info0, rope_nodes_or_crossing_info1, cloud0 = None, cloud1 = None, corr_tile_pattern = np.array([[1]]), rev_perm = None,
                             x_weights = None, reg = .1, rot_reg = np.r_[1e-4, 1e-4, 1e-1], plotting = False, plot_cb = None, Globals=None):
    """
    ...
    SHORT_SEG_LEN = 7#7 #length below which a rope segment can be ignored for topology matching purposes
    ...
    if not rope_closed0 or not rope_closed1:
        if not rope_closed0 and rope_closed1:# or (crossings0 == crossings1 and cross_pairs0 == cross_pairs1)): #also check pairs?
            ...
            if len(crossings0) > len(crossings1):
                ...
                if crossings_links_inds0[0] < SHORT_SEG_LEN:
                    rope_nodes_crossing_infos0.append((rope_nodes0,) + knot_classifier.close_rope(crossings0, crossings_links_inds0, cross_pairs0, 0) + (True,))
            elif len(crossings0) < len(crossings1):
                if len(rope_nodes1)-crossings_links_inds1[-1] < SHORT_SEG_LEN:
                    rope_nodes_crossing_infos1.append((rope_nodes1,) + knot_classifier.unclose_rope(crossings1, crossings_links_inds1, cross_pairs1, -1) + (True,))
        ...
        if not rope_closed1 and rope_closed0:
            ...
            if len(crossings1) > len(crossings0) and crossings1 != [-1,1,1,-1]:
                ...
                #     ipy.embed()
                if len(rope_nodes1)-crossings_links_inds1[-1] < SHORT_SEG_LEN:
                    rope_nodes_crossing_infos1.append((rope_nodes1,) + knot_classifier.close_rope(crossings1, crossings_links_inds1, cross_pairs1, -1) + (True,))
                if crossings_links_inds1[0] < SHORT_SEG_LEN:
                    rope_nodes_crossing_infos1.append((rope_nodes1,) + knot_classifier.close_rope(crossings1, crossings_links_inds1, cross_pairs1, 0) + (True,))
            elif len(crossings1) < len(crossings0):
                if len(rope_nodes0)-crossings_links_inds0[-1] < SHORT_SEG_LEN:
                    rope_nodes_crossing_infos0.append((rope_nodes0,) + knot_classifier.unclose_rope(crossings0, crossings_links_inds0, cross_pairs0, -1) + (True,))
                if crossings_links_inds0[0] < SHORT_SEG_LEN:
                    rope_nodes_crossing_infos0.append((rope_nodes0,) + knot_classifier.unclose_rope(crossings0, crossings_links_inds0, cross_pairs0, 0) + (True,))

        for rope_nodes_crossing_info0 in rope_nodes_crossing_infos0:
            rope_nodes_crossing_info1 = rope_nodes_crossing_infos1[0]
            f_var, corr_nm_var = tps_segment_registration(rope_nodes_crossing_info0, rope_nodes_crossing_info1, cloud0 = cloud0, cloud1 = cloud1, corr_tile_pattern = corr_tile_pattern, 
                                                          x_weights = x_weights, reg = reg, rot_reg = rot_reg, plotting = False, plot_cb = None, Globals=Globals)
        ...
        for rope_nodes_crossing_info1 in rope_nodes_crossing_infos1:
            rope_nodes_crossing_info0 = rope_nodes_crossing_infos0[0]
            f_var, corr_nm_var = tps_segment_registration(rope_nodes_crossing_info0, rope_nodes_crossing_info1, cloud0 = cloud0, cloud1 = cloud1, corr_tile_pattern = corr_tile_pattern, 
                                                          x_weights = x_weights, reg = reg, rot_reg = rot_reg, plotting = False, plot_cb = None, Globals=Globals)
            f_variations.append(f_var)
            corr_nm_variations.append(corr_nm_var)

    #try a version with Reidemeister move 1 applied (i.e. simple loops removed)
    rdm1_0 = knot_classifier.remove_consecutive_crossings(crossings0, crossings_links_inds0, cross_pairs0)
    rdm1_1 = knot_classifier.remove_consecutive_crossings(crossings1, crossings_links_inds1, cross_pairs1)
    if rdm1_0 != (crossings0, crossings_links_inds0, cross_pairs0) and len(crossings0) > 2:
        rope_nodes_crossing_info0 = (rope_nodes0,) + rdm1_0 + (True,)
        rope_nodes_crossing_info1 = rope_nodes_crossing_infos1[0]
        f_var, corr_nm_var = tps_segment_registration(rope_nodes_crossing_info0, rope_nodes_crossing_info1, cloud0 = cloud0, cloud1 = cloud1, corr_tile_pattern = corr_tile_pattern, 
                                                      x_weights = x_weights, reg = reg, rot_reg = rot_reg, plotting = False, plot_cb = None, Globals=Globals)
        f_variations.append(f_var)
        corr_nm_variations.append(corr_nm_var)
    if rdm1_1 != (crossings1, crossings_links_inds1, cross_pairs1) and len(crossings0) > 2:
        rope_nodes_crossing_info1 = (rope_nodes1,) + rdm1_1 + (True,)
        rope_nodes_crossing_info0 = rope_nodes_crossing_infos0[0]
        f_var, corr_nm_var = tps_segment_registration(rope_nodes_crossing_info0, rope_nodes_crossing_info1, cloud0 = cloud0, cloud1 = cloud1, corr_tile_pattern = corr_tile_pattern, 
                                                      x_weights = x_weights, reg = reg, rot_reg = rot_reg, plotting = False, plot_cb = None, Globals=Globals)
        f_variations.append(f_var)
    ...
    crossings0 = np.array(crossings0, dtype=int)
    crossings1 = np.array(crossings1, dtype=int)
    crossings_links_inds0 = np.array(crossings_links_inds0, dtype=int)
    crossings_links_inds1 = np.array(crossings_links_inds1, dtype=int)
            ...
            for reversed_rope_points1 in reversed_rope_points1_variations:
                if reversed_rope_points1:
                    ...
                    if rev_perm is None:
                        ...
                        rev_perm = rev_perm[::-1]
                        rev_perm = np.r_[rev_perm[int(len(rev_perm)/2)-1:,:], rev_perm[:int(len(rev_perm)/2)-1,:]]
                ...
                # if Globals != None:
                #     print "\nfinished fitting ThinPlateSpline\n"
                #     plot_cloud0 = Globals.env.drawlinestrip(cloud0_var[len(cloud0_var)*.25:], 5, (0,1,0,1))
                #     plot_cloud0l = Globals.env.drawlinestrip(cloud0_var[:len(cloud0_var)*.25], 15, (0,.5,0,1))
                #     plot_cloud1 = Globals.env.drawlinestrip(cloud1_var[len(cloud1_var)*.25:], 5, (0,0,1,1))
                #     plot_cloud1l = Globals.env.drawlinestrip(cloud1_var[:len(cloud1_var)*.25], 15, (0,0,.5,1))
                #     plot_cloud2 = Globals.env.drawlinestrip(f_var.transform_points(cloud0_var[len(cloud0_var)*.25:]), 5, (1,0,0,1))
                #     plot_cloud2l = Globals.env.drawlinestrip(f_var.transform_points(cloud0_var[:len(cloud0_var)*.25]), 10, (.5,0,0,1))
                #     # Globals.viewer.Step()
                #     # ipy.embed()

                f_variations.append(f_var)
                corr_nm_variations.append(corr_nm_var)
    elif type(cross_pairs0) != type(cross_pairs1):
        print "\ntype mismatch\n\n\n"
        ipy.embed()
    # else: #fit one variation with no regard to crossings info
    #     if rev_perm is None:
    #         rev_perm = np.eye(len(corr_tile_pattern))
    #         rev_perm = rev_perm[::-1]
    #         rev_perm = np.r_[rev_perm[int(len(rev_perm)/2)-1:,:], rev_perm[:int(len(rev_perm)/2)-1,:]]
    #     corr_nm_var = calc_segment_corr(rope_nodes1, np.array([0,67]), np.array([0,67]))
    #     corr_nm_var_aug = tile(corr_nm_var, rev_perm.dot(corr_tile_pattern))
    ...
    if not f_variations:
        ...
        if len(f_variations) > 1:
            ...
            # f_costs = [registration.tps_reg_cost(f_var) for f_var in f_variations]
            f_costs = [f_var._cost for f_var in f_variations]
            ...
            f_variations, corr_nm_variations = zip(*[(f_var, corr_nm_var) for (f_cost, f_var, corr_nm_var) in sorted(zip(f_costs, f_variations, corr_nm_variations))])
            # f_variations, corr_nm_variations = zip(*[(f_var, corr_nm_var) for (f_var, corr_nm_var) in sorted(zip(f_variations, corr_nm_variations))])
        f = f_variations[0]
...
def main():
    ...
    from hd_rapprentice import clouds, plotting_plt
    import registration
    import time

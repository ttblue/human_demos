...
def record_demo (bag_command, camera_commands, use_voice=True):
    """
    ...
    try:
        ...
        if use_voice:
            subprocess.call("espeak -v en 'Say finish/cancel recording when finished record.'", shell=True)
            while True:
...
def record_pipeline ( demo_type, calib_file = "", 
    ...
    """
    global cmd_checker, camera_types, demo_type_dir, master_file, demo_num, demo_info
...
if __name__ == '__main__':
    ...
    parser.add_argument("--single_demo", help="Single or multiple demos?", action="store_true", default=False, type=bool)

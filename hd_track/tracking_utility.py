...
def plot_kalman_(X_kf, X_ks, X_ar1, vs_ar1, X_ar2, vs_ar2, X_hy, vs_hy, plot_commands):
...
def plot_kalman(data_file, calib_file, lr, freq, use_spline=False, customized_shift=None, single_camera=False, plot_commands='s12fh'):
    '''
    input: data_file
    lr: 'l' or 'r'
    freq
    ...
    plot_kalman_(X_kf[:,1:], X_ks[:,1:], X_ar1, indices_ar1, X_ar2, indices_ar2, X_hy, indices_hy, plot_commands)
    plt.show()
    
    
def rviz_kalman(demo_dir, bag_file, data_file, calib_file, lr, freq, use_rgbd=False, use_smoother=False, use_spline=False, customized_shift=None, single_camera=False):
    '''
    ...
    if use_smoother:
        ...
        T_filt = state_to_hmat(list(X_kf.T))

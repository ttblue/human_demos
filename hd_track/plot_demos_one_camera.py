...
def load_data(fname):
    ...
    return (cam1_ts, cam1_tfs, hydra_ts, hydra_tfs)


def relative_time_streams(fname, freq):
    c1_ts, c1_tfs, hy_ts, hy_tfs = load_data(fname)
    dt =1./freq
    ...
    return tmin, tmax, nsteps, ar1_strm, hy_strm


def get_first_state(dt, c1_ts, c1_tfs, hy_ts, hy_tfs):
    """
...
def setup_kalman(fname, freq=30.):
    ...
    # initialize KF:
    x0, S0 = get_first_state(dt, c1_ts, c1_tfs, hy_ts, hy_tfs)
    KF     = kalman()
    ...
    return (KF, nsteps, tmin, ar1_strm, hy_strm)
...
def run_kalman_filter(fname, freq=30., use_spline=False, use_hydra=True):
    ...
    """
    dt = 1/freq
    KF, nsteps, tmin, ar1_strm, hy_strm = setup_kalman(fname, freq)
    ...
    for i in xrange(nsteps):
        if use_hydra:
            KF.register_observation(dt*i, soft_next(ar1_strm), soft_next(smooth_hy)) 
        else:
            KF.register_observation(dt*i, soft_next(ar1_strm), None)
...
def plot_kalman(X_kf, X_ks, X_ar1, vs_ar1, X_hy, vs_hy, plot_commands='fs12h'):
    """
...
def main_plot():
    ...
    _, _, _, ar1_strm, hy_strm = relative_time_streams(demo_fname +'.data', freq)
    ...
    plot_kalman(X_kf[:,1:], X_ks[:,1:], X_ar1, vs_ar1, X_hy, vs_hy, 's12fh')
    plt.show()
